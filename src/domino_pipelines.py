...
DOMINO_PIPELINES: list[DominoPipeline] = [
    ...
    DominoPipeline(
        ...
        steps=[
            DominoStep("cpu_bench", "bash:top -bn1 | grep 'Cpu(s)' | awk '{print 100 - $8}' | head -1", "bash"),
            DominoStep("mem_bench", "bash:free -m | awk '/Mem:/{printf \"%.1f Go libres\", $7/1024}'", "bash"),
            DominoStep("gpu_bench", "bash:nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader", "bash"),
            DominoStep("io_bench", "bash:iostat -d 1 2 | tail -n +4 | awk 'NF>0{print $2\" kB/s\"}' | tail -1", "bash"),
